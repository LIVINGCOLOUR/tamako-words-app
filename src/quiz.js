(function () {
  ...
  function normalizeCardImage(value){const text=String(value||"").trim().replace(/^\.\//,"");if(!text)return"";if(/^https?:\/\//i.test(text)||text.startsWith("//"))return"";return /^assets\/cards\/[^?#]+\.(png|jpe?g|webp|gif|svg)([?#].*)?$/i.test(text)?text:"";}
  function normalizeWord(raw,index){const promptText=String(raw.promptText||raw.word||"");const category=String(raw.category||raw.cat||"");const rawImage=String(raw.image||"");const cardImage=normalizeCardImage(raw.imagePath||raw.cardImage||raw.image);const emoji=String(raw.emoji||(!cardImage?rawImage:""));const categoryPrefix=raw.mode==="ja_life"?"ja":"en";const legacyKey=String(raw.legacyKey||raw.word||promptText);return{id:String(raw.id||`${categoryPrefix}_${slugify(promptText,"word")}_${index+1}`),mode:raw.mode||"english",level:Number(raw.level||1),category,promptText,reading:String(raw.reading||raw.kana||promptText),answerText:String(raw.answerText||raw.meaning||raw.ja||promptText),image:rawImage||emoji,cardImage,emoji,imageAlt:String(raw.imageAlt||raw.alt||""),audioText:String(raw.audioText||raw.word||promptText),legacyKey};}
